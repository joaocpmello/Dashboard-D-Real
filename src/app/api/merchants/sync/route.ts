...
export async function POST(req: NextRequest) {
  try {
    ...
    try {
      ...
      // Diagnóstico detalhado de erros da camada de serviço/infra

      // 1. Erros de Criptografia (AES/Key missing)
      ...
      // 2. Erros de Autenticação iFood (401/400)
      ...
      // 3. Erros de Rede / Timeout / Gateway
    ...
    console.error('[MERCHANTS SYNC CRITICAL]:', err);

    return NextResponse.json({
      success: false,
      error: err instanceof Error ? err.message : 'Falha interna ao processar a sincronização de lojas.',
